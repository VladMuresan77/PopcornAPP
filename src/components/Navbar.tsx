...
const Navbar: React.FC<Props> = ({ query, onChange }) => {
  return (
    <nav className="bg-slate-800 shadow-lg fixed top-0 left-0 w-full z-50 px-4 md:px-16 py-2 flex flex-col md:flex-row items-center justify-between gap-4">
      {/* Logo popcornApp */}
      ...
      <div className="flex items-center gap-1 md:gap-2 text-sm md:text-base">
        ...
        <Link
          to="/login"
          className="flex items-center gap-0.5 py-1 px-5 text-lg font-light text-white hover:text-sky-900 hover:bg-slate-700 rounded-2xl transition duration-300"
        >
